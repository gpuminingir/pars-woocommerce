...
global $g_PARSWC__plugin_directory_url;
$g_PARSWC__plugin_directory_url = plugins_url ('', __FILE__);

global $g_PARSWC__cron_script_url;
$g_PARSWC__cron_script_url = $g_PARSWC__plugin_directory_url . '/parswc-cron.php';
...
$g_PARSWC__config_defaults = array (
...
//===========================================================================
function PARSWC__GetPluginNameVersionEdition($please_donate = false) // false to turn off
{
  $return_data = '<h2 style="border-bottom:1px solid #DDD;padding-bottom:10px;margin-bottom:20px;">' .
            PARSWC_PLUGIN_NAME . ', version: <span style="color:#EE0000;">' .
            PARSWC_VERSION. '</span>' .
          '</h2>';
  ...
  if ($please_donate)
  {
    $return_data .= '<p style="border:1px solid #890e4e;padding:5px 10px;color:#004400;background-color:#FFF;"><u>Please donate PARS to</u>:&nbsp;&nbsp;<span style="color:#d21577;font-size:110%;font-weight:bold;"></span></p>';
  }
...
function PARSWC__withdraw ()
{
    ...
    {
      return "ParsiCoin address is not found in wallet.";
    } else {
      ...
      $send_amount = (floor( $address_balance / 10000 ) * 10000 ) - 20000; // Only allows sending 4 decimal places
...
//===========================================================================
function PARSWC__get_settings ($key=false)
{
  global   $g_PARSWC__plugin_directory_url;
  global   $g_PARSWC__config_defaults;

  $parswc_settings = get_option (PARSWC_SETTINGS_NAME);
  if (!is_array($parswc_settings))
    $parswc_settings = array();

  if ($key)
    return (@$parswc_settings[$key]);
  else
    return ($parswc_settings);
}
//===========================================================================

//===========================================================================
function PARSWC__update_settings ($parswc_use_these_settings=false, $also_update_persistent_settings=false)
{
   if ($parswc_use_these_settings)
      {
      // if ($also_update_persistent_settings)
      //   PARSWC__update_persistent_settings ($parswc_use_these_settings);

      update_option (PARSWC_SETTINGS_NAME, $parswc_use_these_settings);
      return;
      }
   ...
   foreach ($g_PARSWC__config_defaults as $k=>$v)
      {
      if (isset($_POST[$k]))
         {
         if (!isset($parswc_settings[$k]))
            $parswc_settings[$k] = ""; // Force set to something.
         PARSWC__update_individual_parswc_setting ($parswc_settings[$k], $_POST[$k]);
         }
      // If not in POST - existing will be used.
      }

  update_option (PARSWC_SETTINGS_NAME, $parswc_settings);
}
//===========================================================================

//===========================================================================
// Takes care of recursive updating
function PARSWC__update_individual_parswc_setting (&$parswc_current_setting, $parswc_new_setting)
{
   if (is_string($parswc_new_setting))
      $parswc_current_setting = PARSWC__stripslashes ($parswc_new_setting);
   else if (is_array($parswc_new_setting))  // Note: new setting may not exist yet in current setting: curr[t5] - not set yet, while new[t5] set.
      {
      // Need to do recursive
      foreach ($parswc_new_setting as $k=>$v)
         {
         if (!isset($parswc_current_setting[$k]))
            $parswc_current_setting[$k] = "";   // If not set yet - force set it to something.
         PARSWC__update_individual_parswc_setting ($parswc_current_setting[$k], $v);
         }
      }
   else
      $parswc_current_setting = $parswc_new_setting;
}
//===========================================================================
...
function PARSWC__reset_partial_settings ($also_reset_persistent_settings=false)
{
   global   $g_PARSWC__config_defaults;
   ...
   foreach ($_POST as $k=>$v)
      {
      if (isset($g_PARSWC__config_defaults[$k]))
         {
         if (!isset($parswc_settings[$k]))
            $parswc_settings[$k] = ""; // Force set to something.
         PARSWC__update_individual_parswc_setting ($parswc_settings[$k], $g_PARSWC__config_defaults[$k]);
         }
      }
  ...
  //   PARSWC__update_persistent_settings ($parswc_settings);
}
//===========================================================================

//===========================================================================
function PARSWC__reset_all_settings ($also_reset_persistent_settings=false)
{
  global   $g_PARSWC__config_defaults;

  update_option (PARSWC_SETTINGS_NAME, $g_PARSWC__config_defaults);

  // if ($also_reset_persistent_settings)
  //   PARSWC__reset_all_persistent_settings ();
}
//===========================================================================

//===========================================================================
// Recursively strip slashes from all elements of multi-nested array
function PARSWC__stripslashes (&$val)
{
   if (is_string($val))
...
*/
function PARSWC__create_database_tables ($parswc_settings)
{
  global $wpdb;
  ...
  if($wpdb->get_var("SHOW TABLES LIKE '$pars_payments_table_name'") != $pars_payments_table_name)
      $b_first_time = true;
  else
      $b_first_time = false;
 ...
 //----------------------------------------------------------
 // Create tables
  $query = "CREATE TABLE IF NOT EXISTS `$pars_payments_table_name` (
    `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `pars_address` char(97) NOT NULL,
    `pars_payment_id` char(64) NOT NULL,
    ...
    PRIMARY KEY (`id`),
    UNIQUE KEY `pars_payment_id` (`pars_payment_id`),
...
function PARSWC__delete_database_tables ()
{
  global $wpdb;

  $pars_payments_table_name    = $wpdb->prefix . 'parswc_pars_payments';

  $wpdb->query("DROP TABLE IF EXISTS `$pars_payments_table_name`");
}
//===========================================================================
